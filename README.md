...
```
├── cmd/main.go                    # entrypoint (config + wire + serve) — DONE
├── config/                        # config.go, config.yaml, secret.env.example — DONE
├── internal/
│   ├── handlers/rest/
│   │   ├── router.go              # routes — DONE
│   │   ├── health_check.go        # /health /ready — DONE
│   │   └── loan.go                # TODO: 3 endpoint handlers (return 501 now)
│   ├── models/
│   │   ├── common.go              # response wrappers — DONE
│   │   └── loan.go                # DTOs + loan purpose list — DONE
│   ├── repositories/adaptor/
│   │   ├── adaptor.go             # repository interface — DONE
│   │   ├── entities/loan.go       # MySQL row struct — DONE
│   │   ├── mocks/                 # mockery output (go generate)
│   │   └── mysql/
│   │       ├── db.go              # sql.Open + ping — DONE
│   │       └── loan.go           # TODO: 3 repo methods (return errUnimplemented)
│   └── services/
│       ├── loan.go               # TODO: 3 service methods (return ErrNotImplemented)
│       ├── common.go             # errors + response helpers — DONE
│       └── mocks/                # mockery output (go generate)
├── _compose/compose.yaml          # MySQL docker compose — DONE
├── Dockerfile                     # DONE
├── Makefile                       # DONE
└── go.mod
```
